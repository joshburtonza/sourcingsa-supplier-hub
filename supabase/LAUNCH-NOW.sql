-- ZA Supplier Hub — COMBINED GO-LIVE MIGRATION (base roles/RLS + v2)
-- Project: jxqyxovqagwttjltagon (Lovable-managed Supabase)
-- HOW: Supabase Dashboard -> SQL Editor -> New query -> paste ALL of this -> Run.
-- Probed 2026-06-03: scaffold tables exist; user_roles + v2 tables missing;
--   products currently world-readable to anon (PART 1 fixes this). Both parts idempotent.

-- ============ PART 1 — roles + RLS lockdown (20260601090000) ============
...
-- ============ PART 2 — v2 schema: ordering/admin/requests/tickets (20260602120000) ============
...
-- ============ PART 3 — (run only if admin doesn't work after you log in) ============
-- The seed above grants admin to josh@amalfiai.com ONLY if that auth user already exists.
-- If you signed up AFTER running this, re-run just this block:
-- insert into public.user_roles (user_id, role) select id, 'admin'  from auth.users where lower(email)='josh@amalfiai.com' on conflict do nothing;
-- insert into public.user_roles (user_id, role) select id, 'approved' from auth.users where lower(email)='josh@amalfiai.com' on conflict do nothing;
