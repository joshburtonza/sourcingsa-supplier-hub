  ...
  return (
    <section className="grain relative overflow-hidden px-4 pt-16 pb-24 sm:px-6 sm:pt-24 lg:px-8 lg:pt-28">
      <div className="relative mx-auto flex max-w-5xl flex-col items-center text-center fade-in-up">
        <div className="inline-flex items-center gap-2 rounded-full border border-[color:var(--primary)]/40 bg-black px-4 py-1.5 text-xs">
        ...
        <a href={CHECKOUT_URL} target="_blank" rel="noreferrer"
          className="mt-10 inline-flex items-center gap-2 rounded-full bg-[color:var(--primary)] px-7 py-4 text-base font-bold text-black transition-colors hover:bg-[color:var(--primary-hover)] glow-btn">
        ...
        <div className="mt-12 flex flex-col items-center gap-4">
          <div className="flex items-center -space-x-2">
            ...
            <div className="grid h-9 w-9 place-items-center rounded-full border-2 border-black bg-[color:var(--primary)] text-xs font-bold text-black">+</div>
          </div>
      ...
      <ul className="mt-6 grid gap-2.5 sm:grid-cols-2">
        {bullets.map((b) => (
          <li key={b} className="flex items-center gap-2 text-sm text-white">
            <Check className="h-4 w-4 text-[color:var(--primary)]" strokeWidth={3} />
            {b}
          </li>
        ))}
      </ul>
    ...
    <section className="relative px-4 py-24 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-7xl">
        <div className="grid gap-5 lg:grid-cols-3">
          {stats.map((s) => (
            <div key={s.value}
              className="flex flex-col rounded-2xl border border-[#1F1F1F] bg-[#111111] p-8 text-center">
              ...
              <span className="mx-auto mt-6 inline-flex w-fit items-center gap-1.5 rounded-full bg-[color:var(--primary)]/10 px-3 py-1 text-xs font-semibold text-[color:var(--primary)]">
  ...
  return (
    <section id="pricing" className="relative px-4 py-24 sm:px-6 lg:px-8">
      <div className="relative mx-auto max-w-md">
        ...
        <div className="mt-10 rounded-3xl border border-[color:var(--primary)]/60 bg-[#111111] p-8 glow-btn">
          <div className="text-xs font-bold uppercase tracking-[0.2em] text-[color:var(--primary)]">
            Member Access
          </div>
          ...
          <a href={CHECKOUT_URL} target="_blank" rel="noreferrer"
            className="mt-8 flex w-full items-center justify-center gap-2 rounded-full bg-[color:var(--primary)] px-6 py-4 text-base font-bold text-black transition-colors hover:bg-[color:var(--primary-hover)]">
            Get Instant Access
            <ArrowRight className="h-4 w-4" />
          </a>
...
function SectionEyebrow({ text }: { text: string }) {
  return (
    <div className="inline-flex items-center gap-2 rounded-full border border-[#1F1F1F] bg-[#111111] px-3 py-1">
      <span className="h-1.5 w-1.5 rounded-full bg-[color:var(--primary)]" />
    ...
    <footer className="border-t border-[#1F1F1F] bg-black px-4 py-10 sm:px-6 lg:px-8">
      <div className="mx-auto flex max-w-7xl flex-col items-center justify-between gap-4 sm:flex-row">
        ...
        <div className="flex items-center gap-6 text-xs">
          <Link to="/login" className="text-[#A1A1AA] hover:text-white">
            Member Login
          </Link>
